...
export default function MembershipPage() {
    ...
    const [pendingSubscriptionData, setPendingSubscriptionData] =
        useState(null);
    ...
    const handleSubscribe = async (planType = 'supporter') => {
        ...
        if (!selectedPlan) {
            openToast(
                'toast',
                true,
                t('membership.please_select_plan'),
                'warning'
            );
            return;
        }
        ...
        if (activeSubscription) {
            openToast(
                'toast',
                true,
                t('membership.active_subscription'),
                'info'
            );
            return;
        }
        ...
        if (paymentStatus === 'waiting_verification') {
            openToast(
                'toast',
                true,
                t('membership.waiting_verification'),
                'warning'
            );
            return;
        }
    ...
    const handleContinueSubscription = async () => {
        ...
        try {
            ...
            const planType =
                pendingSubscriptionData?.planType ||
                pendingSubscriptionData?.planType;
            const payloadType =
                planType === 'supporter' ? 'participant' : 'contributor';
                ...
                <div className="relative z-10 w-full max-w-6xl mx-auto">
                    <div className="flex flex-col items-center justify-center gap-6 px-4 mb-8 lg:flex-row lg:items-start lg:justify-center lg:gap-8 sm:px-6">
                        ...
                        <MembershipCards
                            ...
                            onSelectPlan={(plan) =>
                                handleSelectPlan(plan, 'supporter')
                            }
